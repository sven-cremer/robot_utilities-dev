...
void ArmsCartesian::loadGainParameters(){
	...
	for(int i=0;i<NUMBER_OF_GAINS;i++)
	{
		...
		if(!(nh.getParam(zone + "/name", gain_tmp.name))){ROS_WARN("ArmsCartesian - Gain Parameter Not found.");}
		if(!(nh.getParam(zone + "/t_p",  gain_tmp.t_p))) {ROS_WARN("ArmsCartesian - Gain Parameter Not found.");}
		if(!(nh.getParam(zone + "/t_d",  gain_tmp.t_d))) {ROS_WARN("ArmsCartesian - Gain Parameter Not found.");}
		if(!(nh.getParam(zone + "/r_p",  gain_tmp.r_p))) {ROS_WARN("ArmsCartesian - Gain Parameter Not found.");}
		if(!(nh.getParam(zone + "/r_d",  gain_tmp.r_d))) {ROS_WARN("ArmsCartesian - Gain Parameter Not found.");}
